    ...
    <div className='wrapper' style={{position:'relative',margin:"40px 200px 40px 200px"}} >
        ...
        <div className='header' style={{zIndex: 1,display:"flex",width:"100%",height:'400px', margin:'0px 0px -50px 0px',justifyContent:"space-around"}}>
          ...
          <div style={{position:'relative'}}>
            ...
            {data ? (
            <Radia num={data?.emp} numTotal={data?.empTotal} title="จำนวนพนักงานขับรถโม่ที่เข้าปฎิบัติงาน (คน)" colors={["#4DB86B"]}/>
            ) : (null) }
          </div>
          ...
          <div style={{position:'relative'}}>
            ...
            {data ? (
            <Radia num={data?.truck} numTotal={data?.truckTotal} title="จำนวนรถโม่ที่เข้าปฎิบัติงาน (คัน)" colors={["#FF9C64"]}/>
            ) : (null) }
          </div>
          ...
          <div style={{position:'relative'}}>
            ...
            {data ? (
            <Radia num={data?.goods} numTotal={data?.goodsTotal} title="จำนวนผลิตรวมทั้งหมด (คิว)" colors={["#2C90EE"]}/>
            ) : (null) }
        ...
        <div className='content' style={{position:'relative',display:"flex",paddingTop:'',columnGap:"20px"}}>
            ...
            <div className='box' style={{zIndex: 2 ,position:'relative'}} >
                <div style={{position:'relative'}}>
                    ...
                    {/* <p style={{position:'absolute',top:'80px',right:'25px',fontSize:'30px',fontWeight:'bold'}}>เที่ยว</p> */}
                </div>
                <br/>
                <div style={{position:'relative'}}>
                    <Box  num={data?.q} title="จำนวนคิวทั้งหมด (คิว)" suffix="คิว" colorF="#2C90EE"/>
                    {/* <p style={{position:'absolute',top:'80px',right:'30px',fontSize:'30px',fontWeight:'bold'}}>คิว</p> */}
                </div>
